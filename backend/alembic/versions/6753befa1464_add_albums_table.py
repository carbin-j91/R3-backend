...
down_revision: Union[str, Sequence[str], None] = '9446af16ff9b'
...
def upgrade() -> None:
    ...
    op.drop_table('faces')
    op.alter_column('course_attempts', 'similarity_score',
               existing_type=sa.DOUBLE_PRECISION(precision=53),
               nullable=True)
    op.alter_column('course_attempts', 'is_successful',
               existing_type=sa.BOOLEAN(),
               nullable=False)
    op.alter_column('course_attempts', 'attempted_at',
               existing_type=postgresql.TIMESTAMP(),
               nullable=False)
    op.drop_constraint(op.f('course_attempts_course_id_fkey'), 'course_attempts', type_='foreignkey')
    op.create_foreign_key(None, 'course_attempts', 'courses', ['course_id'], ['id'], ondelete='CASCADE')
    op.alter_column('courses', 'created_at',
               existing_type=postgresql.TIMESTAMP(),
               nullable=False)
    op.create_index('ix_courses_status', 'courses', ['status'], unique=False)
    op.create_index('ix_courses_visibility', 'courses', ['visibility'], unique=False)
    op.create_unique_constraint('uq_courses_user_name', 'courses', ['user_id', 'name'])
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('courses', sa.Column('duration', sa.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=False))
    op.drop_constraint('uq_courses_user_name', 'courses', type_='unique')
    op.drop_index('ix_courses_visibility', table_name='courses')
    op.drop_index('ix_courses_status', table_name='courses')
    op.alter_column('courses', 'created_at',
               existing_type=postgresql.TIMESTAMP(),
               nullable=True)
    op.drop_constraint(None, 'course_attempts', type_='foreignkey')
    op.create_foreign_key(op.f('course_attempts_course_id_fkey'), 'course_attempts', 'courses', ['course_id'], ['id'])
    op.alter_column('course_attempts', 'attempted_at',
               existing_type=postgresql.TIMESTAMP(),
               nullable=True)
    op.alter_column('course_attempts', 'is_successful',
