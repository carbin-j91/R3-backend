...
async def create_run(*, db: AsyncSession = Depends(deps.get_db), run_in: schemas.RunCreate, current_user: models.User = Depends(deps.get_current_user)) -> models.Run:
...
async def update_run(run_id: uuid.UUID, run_in: schemas.RunUpdate, db: AsyncSession = Depends(deps.get_db), current_user: models.User = Depends(deps.get_current_user)):
    run = await crud.run.get_run(db=db, id=run_id, user_id=current_user.id)
    if not run:
        raise HTTPException(status_code=404, detail="Run not found")
